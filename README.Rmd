...
To create a backwards dispersion model, simply set `direction = "backward"` in the `add_dispersion_params()` function and use an `end_time`
that is earlier than the `start_time`. The emission source will be a receptor and the model will run backwards in time.


```R
backward_dispersion_model <-
  create_dispersion_model() %>%
  add_source(
    name = "particle",
    lat = 49.0, lon = -123.0, height = 50,
...
docker run -p 8787:8787 -e ROOT=true -e DISABLE_AUTH=true -v $(pwd):/home/rstudio/kitematic/ splitr
```
